...
from similarity_metrics import calculate_all_similarity_metrics # Renamed function
...
def compute_for_single_combination(params):
    ...
    print(f'Running Case {case_number} - Method: {method} - Sigma: {sigma_option} - Normalize: {normalize} - Empirical: {empirical_method}')
    ...
    mu_s1, actual_sigma_s1 = compute_membership_functions(sensor_data1, x_values, method=method, sigma=sigma_option)
    ...
    mu_s2, actual_sigma_s2 = compute_membership_functions(sensor_data2, x_values, method=method, sigma=sigma_for_s2)
    ...
    fitness_s1, empirical_probs1 = compute_fitness_metrics(sensor_data1, mu_s1, x_values, empirical_method=empirical_method)

    # Collect result - Use specific keys from the new similarities dictionary
    result = {
        'Case': case_number, 'Method': method, 'Sigma_Option': sigma_option,
        'Sigma_S1': actual_sigma_s1, # Store potentially different sigmas
        'Sigma_S2': actual_sigma_s2,
        'Normalized': normalize, 'Empirical_Method': empirical_method,
        'Similarity_Jaccard': similarities.get('Jaccard', np.nan), # Example: Get Jaccard
        'Distance_Euclidean': similarities.get('Distance_Euclidean', np.nan), # Example: Get Euclidean Distance
        'MATLAB_Metric1': similarities.get('MATLAB_Metric1', np.nan), # Example: Get MATLAB Metric 1
...
def run_cases_parallel(specific_cases=None, sigma_options=None, n_jobs=-1):
    ...
    empirical_method_options = ['kde'] # Currently only supports 'kde' based on original code
            ...
            for norm in normalization_options:
                for sigma in sigmas_to_run:
                    for emp_method in empirical_method_options:
                         param_list.append({
                             'case_number': case, 'method': method, 'normalize': norm,
                             'sigma_option': sigma, 'empirical_method': emp_method
                         })

    # Run in parallel
    results_list = Parallel(n_jobs=n_jobs)(delayed(compute_for_single_combination)(params) for params in param_list)
                ...
                for sigma_option in sigmas_to_run:
                    for empirical_method in empirical_method_options:
                        ...
                        try:
                            ...
                            residuals_s1 = empirical_probs1 - mu_s1 if empirical_probs1 is not None else np.full_like(mu_s1, np.nan)
                            ...
                            if empirical_probs1 is not None: # Only plot if empirical probs were calculated
                            ...
                            print(f"Error processing Case {case_number}, Method {method}, Sigma {sigma_option}, Norm {normalize}, Emp {empirical_method}: {e}")
                            results.append({
                                'Case': case_number, 'Method': method, 'Sigma_Option': sigma_option,
                                'Sigma_S1': None, 'Sigma_S2': None, # Sigmas might not be available on error
                                'Normalized': normalize, 'Empirical_Method': empirical_method,
                                # Add scale features even on error if possible
                                'Std_Orig_S1': std_orig_s1, 'Range_Orig_S1': range_orig_s1,
                                'Std_Orig_S2': std_orig_s2, 'Range_Orig_S2': range_orig_s2,
                                'Error': str(e)
                            })
...
def plot_membership_functions(case_number, sensor_data1, sensor_data2, x_values, mu_s1, mu_s2, method, sigma, normalize, similarities):
    """
    ...
    plt.fill_between(x_values, np.minimum(mu_s1, mu_s2), alpha=0.3, color='gray', label='Overlap')

    plt.title(f'Case {case_number} - Method: {method.upper()} - Normalized: {normalize} - Sigma: {str(sigma)}')
    plt.xlabel('x')
    plt.ylabel('Membership Degree')
    plt.legend()

    # Prepare the text with specific similarity measures
    overlap_sim = similarities.get('OverlapCoefficient', np.nan) # Use OverlapCoefficient as 'similarity_overlap'
    euclidean_dist = similarities.get('Distance_Euclidean', np.nan) # Use 'Distance_Euclidean'
    ...
    plt.text(0.95, 0.95, similarity_text,
...
def plot_membership_functions_with_residuals(
    ...
    fig.suptitle(f'Case {case_number} - Method: {method} - Sigma Option: {sigma_option} (Actual: {sigma:.4f}) - Norm: {normalize} - Emp: {empirical_method}', fontsize=14)

    # Plot membership function and empirical probabilities
    axes[0].plot(x_values, mu_s, label=f'Membership Function (μ_s)', color='blue')
    axes[0].plot(x_values, empirical_probs, label=f'Empirical Prob ({empirical_method})', color='red', linestyle='--')
    ...
    axes[0].text(0.05, 0.95, fitness_text,
    ...
    # Plot residuals
    axes[1].plot(x_values, residuals, label='Residuals (Empirical - μ_s)', color='green')
    ...
    output_dir = os.path.join("plots", f"sigma_{sigma_option_str}", norm_str, f"case_{case_number}")
