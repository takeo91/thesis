...
def compute_membership_function(sensor_data, x_values=None, sigma=None, num_points=500):
    ...
    if x_values is None:
        ...
        spread = (
            1.0 if data_range < 1e-9 else data_range
        )  # Use a default spread if range is zero
    ...
    if sigma is None:
        sigma_val = (
            default_sigma_ratio * data_range
            if data_range > 1e-9
            else default_sigma_ratio
        )
        ...
        try:
            ...
            raise ValueError(
                f"Invalid sigma string format: {sigma}. Expected 'r<float>'."
            )
    else:
        try:
            ...
            raise ValueError(
                f"Invalid sigma value: {sigma}. Expected float, None, or 'r<float>'."
            )
...
def safe_divide(numerator, denominator, default=0.0):
    ...
    if np.isscalar(denominator):
        ...
        if numerator.shape == denominator.shape:
            ...
            numerator.shape == (1,) or numerator.size == 1
        ):  # Scalar numerator broadcast
            scalar_num = (
...
def similarity_hamming(mu1, mu2):
    ...
    n = len(mu1)
    if n == 0:
        return 1.0  # Identical if no points to compare
...
def distance_chebyshev(mu1, mu2):
    ...
    mu2 = np.asarray(mu2)
    if mu1.size == 0:
        return 0.0  # No difference if empty
...
def similarity_pearson(mu1, mu2):
    ...
    mu2 = np.asarray(mu2)
    if mu1.size < 2:
        return 0.0  # Correlation undefined for less than 2 points
...
def similarity_matlab_metric1(
    data_s1, data_s2, x_values_common, fs_method="nd", sigma_s1=None, sigma_s2=None
):
    """
    ...
    try:
        ...
        IndexError
    ):  # Handle cases where percentile fails (e.g., too few unique points)
        return np.nan
    ...
    if np.sum(mu_s2_calc) < 1e-9:
        return 0.0  # mu_s2 is essentially zero
    ...
    interp_mu_s2 = interp1d(
        x_values_common, mu_s2_calc, kind="linear", bounds_error=False, fill_value=0
    )
...
def calculate_all_similarity_metrics(
    ...
    if mu_s1.shape != mu_s2.shape or mu_s1.shape != x_values.shape:
        raise ValueError(
            "Input shapes mismatch: mu_s1, mu_s2, and x_values must have the same shape."
        )

    if mu_s1.size == 0:  # Handle empty inputs
        print(
            "Warning: Empty membership function(s) provided. Returning empty similarity dict."
        )
        return {}
    ...
    similarities["OverlapCoefficient"] = similarity_overlap_coefficient(
        mu_s1, mu_s2
    )  # Also MATLAB S7
    # MATLAB Equivalents
    ...
    similarities["Similarity_Chebyshev"] = similarity_chebyshev(
        mu_s1, mu_s2
    )  # Also MATLAB L
    # MATLAB Equivalents / Specific
    similarities["MATLAB_S2_W"] = similarity_matlab_S2_W(
        mu_s1, mu_s2
    )  # S2 and W are the same
