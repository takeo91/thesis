...
class RQ2ExperimentConfig:
    ...
    output_dir: str = "results/rq2_classification"
    opportunity_label_type: str = "ML_Both_Arms"  # New parameter for label type selection
        ...
        if self.target_activities is None:
            self.target_activities = {
                "opportunity": opportunity_activities_by_type.get(
                    ...
                    opportunity_activities_by_type["ML_Both_Arms"]
                ),
                "pamap2": [
...
class RQ2Experiment:
    ...
    def load_and_preprocess_dataset(
        ...
        if dataset_name.lower() == "opportunity":
            ...
            try:
                ...
                # Fallback order
                fallback_order = ["ML_Both_Arms", "LL_Right_Arm", "HL_Activity", "Locomotion"]
            ...
            # Map config names to PAMAP2 body part names
            location_map = {"hand": "Hand", "chest": "Chest", "ankle": "Ankle"}
            sensor_type_map = {"accelerometer": "Accelerometer", "gyroscope": "Gyroscope"}
            axis_map = {"x": "X", "y": "Y", "z": "Z"}
            
            for location in sensor_config["sensor_locations"]:
                body_part = location_map.get(location, location.capitalize())
                for sensor_type in sensor_config["sensor_types"]:
                    sensor_name = sensor_type_map.get(sensor_type, sensor_type.capitalize())
                    for axis in sensor_config["axes"]:
                        axis_name = axis_map.get(axis, axis.upper())
                        try:
                            # Try to get data for this combination
                            sensor_col = df.loc[:, idx[sensor_name, body_part, :, axis_name]]
                            if not sensor_col.empty:
                                sensor_data_list.append(sensor_col)
                        except KeyError:
                            continue
            ...
            if sensor_data_list:
                ...
                # Fallback: get all numeric sensor data
                sensor_mask = df.columns.get_level_values('SensorType').isin(['Accelerometer', 'Gyroscope', 'Magnetometer'])
                data = df.loc[:, sensor_mask].values
            ...
            if activity_mask.any():
                ...
                logger.warning(f"No matching activities found. Available: {np.unique(labels)[:10]}")
                # Use all non-other activities as fallback
                activity_mask = (labels != "other").flatten()
                data = data[activity_mask]
                labels = labels[activity_mask]
        ...
        try:
            ...
            activity_names = list(unique_activities)
        except Exception as e:
            logger.error(f"Error getting unique activities: {e}")
            raise
        
        # Create label encoder - convert string labels to integers
        try:
            label_to_int = {label: i for i, label in enumerate(unique_activities)}
            int_to_label = {i: label for label, i in label_to_int.items()}
        except Exception as e:
            logger.error(f"Error creating label encoder: {e}")
            raise
        ...
        try:
            ...
            logger.error(f"Error converting labels to integers: {e}")
            logger.error(f"Labels shape: {labels.shape}, type: {type(labels)}")
            raise
        
        logger.info(f"   ✅ Loaded {len(data)} samples, {data.shape[1]} features")
        logger.info(f"   ✅ Activities: {len(activity_names)} classes")
        logger.info(f"   ✅ Label encoding: {label_to_int}")
        for activity, count in zip(unique_activities, counts):
            logger.info(f"      - {activity}: {count} samples")
        ...
        if self.config.quick_test:
            ...
            logger.info(f"   ⚡ Quick test mode: using {subset_size} samples")
        
        return data, labels_int, activity_names, int_to_label
    ...
    def run_dataset_experiments(
        ...
        try:
            data, labels, activity_names, int_to_label = self.load_and_preprocess_dataset(dataset_name)
        except Exception as e:
        ...
        classification_config = ClassificationConfig(
            ...
            min_samples_per_class=self.config.min_samples_per_activity // 10,  # Windows have fewer samples
            similarity_normalization=True,
            ndg_kernel_type="epanechnikov",  # Use optimized kernel
        ...
        results = run_activity_classification_experiment(
            ...
            experiment_name=f"rq2_{dataset_name}"
        )
        
        # Save dataset-specific results
        if dataset_name == "opportunity" and self.config.opportunity_label_type != "ML_Both_Arms":
            dataset_output_dir = self.output_dir / f"{dataset_name}_{self.config.opportunity_label_type}"
        else:
            dataset_output_dir = self.output_dir / dataset_name
        save_classification_results(results, dataset_output_dir)
        
        # Store results
        dataset_results = {}
        for result in results:
            config_key = f"w{result.window_config.window_size}_o{result.window_config.overlap_ratio}"
            # Add label mapping to metadata
            result.metadata['int_to_label'] = int_to_label
    ...
    def run_all_experiments(self) -> None:
        ...
        for dataset_name in self.config.datasets:
            dataset_results = self.run_dataset_experiments(dataset_name)
            self.results[dataset_name] = dataset_results
        
        # Update metadata
        self.metadata["end_time"] = datetime.now().isoformat()
        self.metadata["total_duration_seconds"] = time.time() - overall_start_time
        
        # Save overall results
        self.save_results()
        
        logger.info(f"\n✅ All experiments completed in {self.metadata['total_duration_seconds']:.1f} seconds")
    
    def save_results(self) -> None:
        """Save all experiment results and metadata."""
        # Save raw results
        with open(self.output_dir / "rq2_all_results.pkl", "wb") as f:
            pickle.dump({
        ...
        # Save metadata
        with open(self.output_dir / "rq2_metadata.json", "w") as f:
    ...
    def create_summary_report(self) -> None:
        ...
        for dataset_name, dataset_results in self.results.items():
            for config_key, result in dataset_results.items():
                ...
                for metric_name, perf in sorted_metrics:
                    summary_data.append({
        ...
        summary_df.to_csv(self.output_dir / "rq2_comprehensive_summary.csv", index=False)
        
        # Perform statistical analysis
        from thesis.exp.rq2_statistical_analysis import (
            perform_rq2_statistical_analysis,
            plot_metric_performance_heatmap,
            plot_statistical_significance,
            plot_activity_performance,
        ...
        try:
            ...
            statistical_results = perform_rq2_statistical_analysis(summary_df)
            
            # Save statistical results
            with open(self.output_dir / "rq2_statistical_analysis.pkl", "wb") as f:
                pickle.dump(statistical_results, f)
            
            # Save metric rankings
            statistical_results['metric_rankings'].to_csv(
                self.output_dir / "rq2_metric_rankings.csv", index=False
            )
            ...
            plot_metric_performance_heatmap(
                statistical_results, 
                output_path=str(self.output_dir / "rq2_performance_heatmap.png")
            )
            
            plot_statistical_significance(
                statistical_results,
                output_path=str(self.output_dir / "rq2_statistical_significance.png")
            )
            
            plot_activity_performance(
                statistical_results,
                output_path=str(self.output_dir / "rq2_activity_performance.png")
            )
            
            # Generate statistical report
            generate_summary_report(
                statistical_results,
                output_path=str(self.output_dir / "rq2_statistical_report.md")
            )
...
def quick_test():
    ...
    config = RQ2ExperimentConfig(
        ...
        window_sizes=[128],  # Just one window size
        overlap_ratios=[0.5],  # Just one overlap
